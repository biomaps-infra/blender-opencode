...
## Integration with blender-api

This skill covers MolecularNodes-specific operations. For general Blender Python, use `blender-api`. When both apply, read from both.

| Task | Which Skill | Specific Reference |
|------|------------|-------------------|
| General bpy scripting | `blender-api` | `core-api.md` |
| Mesh/BMesh manipulation | `blender-api` | `mesh-and-bmesh.md` |
| Custom shader materials for MN objects | `blender-api` | `materials-and-nodes.md` — assign via `mol.object.data.materials.append(mat)` |
| Geometry Nodes concepts (general) | `blender-api` | `geometry-nodes.md` — how to create/link node trees |
| MN custom nodes (Style, Select, Color) | `molecular-nodes` | `nodes-essential.md` |
| Modifiers on MN objects | `blender-api` | `modifiers-and-constraints.md` |
| Animation / keyframes on MN objects | `blender-api` | `handlers-and-timers.md` for frame handlers |
...
| Debugging render issues | Both | `molecular-nodes/gotchas.md` + `blender-api/gotchas-and-pitfalls.md` |
