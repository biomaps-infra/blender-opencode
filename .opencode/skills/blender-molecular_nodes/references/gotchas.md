...
**Fix**: Use `canvas.snapshot()` or `bpy.ops.render.render(write_still=True)` instead. See `blender-core_api/references/operators-reference.md` for render operator details.
...
**Fix**: In Python, call `bpy.context.view_layer.update()` after modifying nodes. In the GUI, just move a node or change any value to trigger a refresh. See `blender-core_api/references/gotchas-and-pitfalls.md` ("Stale Data & Deferred Updates" section) for the depsgraph update pattern.
